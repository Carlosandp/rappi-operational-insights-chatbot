...
│
├── app/
│   ├── streamlit_app.py      # UI principal (dark-mode, on_click buttons)
│   ├── query_parser.py       # LLM → JSON + fallback por reglas + google.genai
│   ├── executor.py           # Motor analítico (pandas) — 6 intents
│   ├── chatbot.py            # LLM → narrativa + fallback determinístico
│   ├── insights.py           # Insights automáticos (6 categorías, 100% determinístico)
│   ├── semantic_layer.py     # Resolución de aliases, métricas, entidades
│   ├── charts.py             # Plotly con AXIS_STYLE y colores explícitos
│   ├── report_generator.py   # Reporte Markdown + HTML (6 secciones)
│   └── utils.py              # Carga, limpieza (sin SettingWithCopyWarning), cache
│
├── data/
│   ├── raw/dummy_data.xlsx        # Dataset original
│   ├── processed/                 # Cache pickle (auto-invalidado por mtime+size)
│   └── metric_dictionary.json     # Aliases, rangos esperados, unidades
│
├── reports/
│   ├── executive_report.html      # Reporte pre-generado
│   └── executive_report.md
│
├── notebooks/eda.ipynb
